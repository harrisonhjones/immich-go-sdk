/*
Immich

Immich API

API version: 2.7.5
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package immich

import (
	"encoding/json"
	"fmt"
)

// checks if the ActivityCreateDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ActivityCreateDto{}

// ActivityCreateDto Activity create
type ActivityCreateDto struct {
	// Album ID
	AlbumId string `json:"albumId" validate:"regexp=^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12})$"`
	// Asset ID (if activity is for an asset)
	AssetId *string `json:"assetId,omitempty" validate:"regexp=^([0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12})$"`
	// Comment text (required if type is comment)
	Comment *string `json:"comment,omitempty"`
	Type ReactionType `json:"type"`
	AdditionalProperties map[string]interface{}
}

type _ActivityCreateDto ActivityCreateDto

// NewActivityCreateDto instantiates a new ActivityCreateDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewActivityCreateDto(albumId string, type_ ReactionType) *ActivityCreateDto {
	this := ActivityCreateDto{}
	this.AlbumId = albumId
	this.Type = type_
	return &this
}

// NewActivityCreateDtoWithDefaults instantiates a new ActivityCreateDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewActivityCreateDtoWithDefaults() *ActivityCreateDto {
	this := ActivityCreateDto{}
	return &this
}

// GetAlbumId returns the AlbumId field value
func (o *ActivityCreateDto) GetAlbumId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.AlbumId
}

// GetAlbumIdOk returns a tuple with the AlbumId field value
// and a boolean to check if the value has been set.
func (o *ActivityCreateDto) GetAlbumIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AlbumId, true
}

// SetAlbumId sets field value
func (o *ActivityCreateDto) SetAlbumId(v string) {
	o.AlbumId = v
}

// GetAssetId returns the AssetId field value if set, zero value otherwise.
func (o *ActivityCreateDto) GetAssetId() string {
	if o == nil || IsNil(o.AssetId) {
		var ret string
		return ret
	}
	return *o.AssetId
}

// GetAssetIdOk returns a tuple with the AssetId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ActivityCreateDto) GetAssetIdOk() (*string, bool) {
	if o == nil || IsNil(o.AssetId) {
		return nil, false
	}
	return o.AssetId, true
}

// HasAssetId returns a boolean if a field has been set.
func (o *ActivityCreateDto) HasAssetId() bool {
	if o != nil && !IsNil(o.AssetId) {
		return true
	}

	return false
}

// SetAssetId gets a reference to the given string and assigns it to the AssetId field.
func (o *ActivityCreateDto) SetAssetId(v string) {
	o.AssetId = &v
}

// GetComment returns the Comment field value if set, zero value otherwise.
func (o *ActivityCreateDto) GetComment() string {
	if o == nil || IsNil(o.Comment) {
		var ret string
		return ret
	}
	return *o.Comment
}

// GetCommentOk returns a tuple with the Comment field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ActivityCreateDto) GetCommentOk() (*string, bool) {
	if o == nil || IsNil(o.Comment) {
		return nil, false
	}
	return o.Comment, true
}

// HasComment returns a boolean if a field has been set.
func (o *ActivityCreateDto) HasComment() bool {
	if o != nil && !IsNil(o.Comment) {
		return true
	}

	return false
}

// SetComment gets a reference to the given string and assigns it to the Comment field.
func (o *ActivityCreateDto) SetComment(v string) {
	o.Comment = &v
}

// GetType returns the Type field value
func (o *ActivityCreateDto) GetType() ReactionType {
	if o == nil {
		var ret ReactionType
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *ActivityCreateDto) GetTypeOk() (*ReactionType, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *ActivityCreateDto) SetType(v ReactionType) {
	o.Type = v
}

func (o ActivityCreateDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ActivityCreateDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["albumId"] = o.AlbumId
	if !IsNil(o.AssetId) {
		toSerialize["assetId"] = o.AssetId
	}
	if !IsNil(o.Comment) {
		toSerialize["comment"] = o.Comment
	}
	toSerialize["type"] = o.Type

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *ActivityCreateDto) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"albumId",
		"type",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varActivityCreateDto := _ActivityCreateDto{}

	err = json.Unmarshal(data, &varActivityCreateDto)

	if err != nil {
		return err
	}

	*o = ActivityCreateDto(varActivityCreateDto)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "albumId")
		delete(additionalProperties, "assetId")
		delete(additionalProperties, "comment")
		delete(additionalProperties, "type")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableActivityCreateDto struct {
	value *ActivityCreateDto
	isSet bool
}

func (v NullableActivityCreateDto) Get() *ActivityCreateDto {
	return v.value
}

func (v *NullableActivityCreateDto) Set(val *ActivityCreateDto) {
	v.value = val
	v.isSet = true
}

func (v NullableActivityCreateDto) IsSet() bool {
	return v.isSet
}

func (v *NullableActivityCreateDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableActivityCreateDto(val *ActivityCreateDto) *NullableActivityCreateDto {
	return &NullableActivityCreateDto{value: val, isSet: true}
}

func (v NullableActivityCreateDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableActivityCreateDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



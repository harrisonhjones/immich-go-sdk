/*
Immich

Immich API

API version: 2.7.5
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package immich

import (
	"encoding/json"
	"fmt"
)

// checks if the AssetStackResponseDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AssetStackResponseDto{}

// AssetStackResponseDto struct for AssetStackResponseDto
type AssetStackResponseDto struct {
	// Number of assets in stack
	AssetCount int32 `json:"assetCount"`
	// Stack ID
	Id string `json:"id"`
	// Primary asset ID
	PrimaryAssetId string `json:"primaryAssetId"`
	AdditionalProperties map[string]interface{}
}

type _AssetStackResponseDto AssetStackResponseDto

// NewAssetStackResponseDto instantiates a new AssetStackResponseDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAssetStackResponseDto(assetCount int32, id string, primaryAssetId string) *AssetStackResponseDto {
	this := AssetStackResponseDto{}
	this.AssetCount = assetCount
	this.Id = id
	this.PrimaryAssetId = primaryAssetId
	return &this
}

// NewAssetStackResponseDtoWithDefaults instantiates a new AssetStackResponseDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAssetStackResponseDtoWithDefaults() *AssetStackResponseDto {
	this := AssetStackResponseDto{}
	return &this
}

// GetAssetCount returns the AssetCount field value
func (o *AssetStackResponseDto) GetAssetCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.AssetCount
}

// GetAssetCountOk returns a tuple with the AssetCount field value
// and a boolean to check if the value has been set.
func (o *AssetStackResponseDto) GetAssetCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AssetCount, true
}

// SetAssetCount sets field value
func (o *AssetStackResponseDto) SetAssetCount(v int32) {
	o.AssetCount = v
}

// GetId returns the Id field value
func (o *AssetStackResponseDto) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *AssetStackResponseDto) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *AssetStackResponseDto) SetId(v string) {
	o.Id = v
}

// GetPrimaryAssetId returns the PrimaryAssetId field value
func (o *AssetStackResponseDto) GetPrimaryAssetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.PrimaryAssetId
}

// GetPrimaryAssetIdOk returns a tuple with the PrimaryAssetId field value
// and a boolean to check if the value has been set.
func (o *AssetStackResponseDto) GetPrimaryAssetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PrimaryAssetId, true
}

// SetPrimaryAssetId sets field value
func (o *AssetStackResponseDto) SetPrimaryAssetId(v string) {
	o.PrimaryAssetId = v
}

func (o AssetStackResponseDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AssetStackResponseDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["assetCount"] = o.AssetCount
	toSerialize["id"] = o.Id
	toSerialize["primaryAssetId"] = o.PrimaryAssetId

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *AssetStackResponseDto) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"assetCount",
		"id",
		"primaryAssetId",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varAssetStackResponseDto := _AssetStackResponseDto{}

	err = json.Unmarshal(data, &varAssetStackResponseDto)

	if err != nil {
		return err
	}

	*o = AssetStackResponseDto(varAssetStackResponseDto)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "assetCount")
		delete(additionalProperties, "id")
		delete(additionalProperties, "primaryAssetId")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableAssetStackResponseDto struct {
	value *AssetStackResponseDto
	isSet bool
}

func (v NullableAssetStackResponseDto) Get() *AssetStackResponseDto {
	return v.value
}

func (v *NullableAssetStackResponseDto) Set(val *AssetStackResponseDto) {
	v.value = val
	v.isSet = true
}

func (v NullableAssetStackResponseDto) IsSet() bool {
	return v.isSet
}

func (v *NullableAssetStackResponseDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAssetStackResponseDto(val *AssetStackResponseDto) *NullableAssetStackResponseDto {
	return &NullableAssetStackResponseDto{value: val, isSet: true}
}

func (v NullableAssetStackResponseDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAssetStackResponseDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


